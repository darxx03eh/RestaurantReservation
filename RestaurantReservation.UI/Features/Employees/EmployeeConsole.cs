...
public sealed class EmployeeConsole(RestaurantReservationDbContext context, IUnitOfWork unitOfWork)
{
    public async Task ListAsync()
    {
        var employees = await unitOfWork.Employees.GetAllAsync();
        if (Ui.ShowEmptyIfNeeded(employees, "employees"))
            return;
    ...
    public async Task ShowRestaurantDetailsViewAsync()
    {
        ...
        if (Ui.ShowEmptyIfNeeded(rows, "employee restaurant details"))
            return;
